...
DifferentialCarPawnSimApi::DifferentialCarPawnSimApi(const Params& params,
	const msr::airlib::CarApiBase::DifferentialCarControls& keyboard_controls, USimpleWheeledVehicleMovementComponent* movement)
	: PawnSimApi(params), params_(params), keyboard_controls_(keyboard_controls)
{
}

void DifferentialCarPawnSimApi::initialize()
{
	PawnSimApi::initialize();

	createVehicleApi(static_cast<ADifferentialCarPawn*>(params_.pawn), params_.home_geopoint);

	//TODO: should do reset() here?
	joystick_controls_ = msr::airlib::CarApiBase::DifferentialCarControls();

}

void DifferentialCarPawnSimApi::createVehicleApi(ADifferentialCarPawn* pawn, const msr::airlib::GeoPoint& home_geopoint)
{
	...
	pawn_api_ = std::unique_ptr<DifferentialCarPawnApi>(new DifferentialCarPawnApi(pawn, getGroundTruthKinematics(), vehicle_api_.get()));
}

std::string DifferentialCarPawnSimApi::getRecordFileLine(bool is_header_line) const
{
	std::string common_line = PawnSimApi::getRecordFileLine(is_header_line);
	if (is_header_line) {
		return common_line +
			"Torque_FL\tTorque_FR\tTorque_RL\tTorque_RR\tHandbrake\t";
	}

	const auto& state = pawn_api_->getDifferentialCarState();

	std::ostringstream ss;
	ss << common_line;
	ss << current_controls_.torque_FL << "\t" << current_controls_.torque_FR << "\t" << current_controls_.torque_RL << "\t";
	ss << state.rpm_RR_0 << "\t" << state.handbrake << "\t";

	return ss.str();
}

//these are called on render ticks
void DifferentialCarPawnSimApi::updateRenderedState(float dt)
{
...
void DifferentialCarPawnSimApi::updateRendering(float dt)
{
	PawnSimApi::updateRendering(dt);

	updateDifferentialCarControls();
...
void DifferentialCarPawnSimApi::updateDifferentialCarControls()
{
    ...
    if (rc_data.is_initialized) {
        ...
        //UAirBlueprintLib::LogMessageString("ERROR ", std::to_string(error in update(SimApi)), LogDebugLevel::Informational);
    }
    else {
    ...
    if (!vehicle_api_->isApiControlEnabled()) {
        ...
        vehicle_api_->setDifferentialCarControls(current_controls_);
        pawn_api_->updateMovement(current_controls_);
    }
    else {
        UAirBlueprintLib::LogMessageString("Control Mode: ", "API", LogDebugLevel::Informational);
        current_controls_ = vehicle_api_->getDifferentialCarControls();
        pawn_api_->updateMovement(current_controls_);
    }
    UAirBlueprintLib::LogMessageString("Torque_FL: ", std::to_string(current_controls_.torque_FL), LogDebugLevel::Informational);
    UAirBlueprintLib::LogMessageString("Torque_FR: ", std::to_string(current_controls_.torque_FR), LogDebugLevel::Informational);
    UAirBlueprintLib::LogMessageString("Torque_RL: ", std::to_string(current_controls_.torque_RL), LogDebugLevel::Informational);
    UAirBlueprintLib::LogMessageString("Torque_RR: ", std::to_string(current_controls_.torque_RR), LogDebugLevel::Informational);
    UAirBlueprintLib::LogMessageString("Handbrake: ", std::to_string(current_controls_.handbrake), LogDebugLevel::Informational);
}

//*** Start: UpdatableState implementation ***/ /
void DifferentialCarPawnSimApi::resetImplementation()
{
...
//physics tick
void DifferentialCarPawnSimApi::update()
{
    pawn_api_->update();
...
void DifferentialCarPawnSimApi::reportState(StateReporter& reporter)
{
