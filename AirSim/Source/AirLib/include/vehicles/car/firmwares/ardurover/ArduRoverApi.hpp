...
namespace airlib
{
    ...
    public:
        //差速车辆
        virtual void setDifferentialCarControls(const DifferentialCarControls& controls) override
        {
            unused(controls);
        }

        virtual void updateDifferentialCarState(const DifferentialCarState& state) override
        {
            last_differential_car_state_ = state;
        }

        virtual const DifferentialCarState& getDifferentialCarState() const override
        {
            return last_differential_car_state_;
        }

        virtual const DifferentialCarControls& getDifferentialCarControls() const override
        {
            return last_differential_controls_;
        }
    ...
    private:
        //差速车辆
        void recvRoverControl_differential()
        {
            // Receive motor data
            RoverControlMessage_Differential pkt;
            int recv_ret = udp_socket_->recv(&pkt, sizeof(pkt), 100);
            while (recv_ret != sizeof(pkt)) {
                if (recv_ret <= 0) {
                    Utils::log(Utils::stringf("Error while receiving rotor control data - ErrorNo: %d", recv_ret), Utils::kLogLevelInfo);
            ...
            last_differential_controls_.torque_FL = pkt.torque_FL;
            last_differential_controls_.torque_FR = pkt.torque_FR;
            last_differential_controls_.torque_RL = pkt.torque_RL;
            last_differential_controls_.torque_RR = pkt.torque_RR;
    ...
    private:
        ...
        struct RoverControlMessage_Differential
        {
            float torque_FL;
            float torque_FR;
            float torque_RL;
        ...
        //差速车辆
        DifferentialCarControls last_differential_controls_;
        DifferentialCarState last_differential_car_state_;
    };
}
} // namespace
