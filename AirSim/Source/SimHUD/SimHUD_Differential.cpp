...
#include "SimHUD_Differential.h"
#include "UObject/ConstructorHelpers.h"
#include "Kismet/KismetSystemLibrary.h"
#include "Misc/FileHelper.h"

#include "Vehicles/Multirotor/SimModeWorldMultiRotor.h"
#include "Vehicles/DifferentialVehicles/SimModeDifferentialCar.h"
#include "Vehicles/ComputerVision/SimModeComputerVision.h"

#include "common/AirSimSettings.hpp"
#include <stdexcept>

ASimHUD_Differential::ASimHUD_Differential()
{
...
void ASimHUD_Differential::BeginPlay()
{
	Super::BeginPlay();
...
void ASimHUD_Differential::Tick(float DeltaSeconds)
{
	if (simmode_ && simmode_->EnableReport)
		widget_->updateDebugReport(simmode_->getDebugReport());
}

void ASimHUD_Differential::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (simmode_)
...
void ASimHUD_Differential::toggleRecordHandler()
{
	simmode_->toggleRecording();
}

void ASimHUD_Differential::inputEventToggleRecording()
{
	toggleRecordHandler();
}

void ASimHUD_Differential::inputEventToggleReport()
{
	simmode_->EnableReport = !simmode_->EnableReport;
	widget_->setReportVisible(simmode_->EnableReport);
}

void ASimHUD_Differential::inputEventToggleHelp()
{
	widget_->toggleHelpVisibility();
}

void ASimHUD_Differential::inputEventToggleTrace()
{
	simmode_->getVehicleSimApi()->toggleTrace();
}

ASimHUD_Differential::ImageType ASimHUD_Differential::getSubwindowCameraType(int window_index)
{
	//TODO: index check
	return getSubWindowSettings().at(window_index).image_type;
}

void ASimHUD_Differential::setSubwindowCameraType(int window_index, ImageType type)
{
	getSubWindowSettings().at(window_index).image_type = type;
	updateWidgetSubwindowVisibility();
}

APIPCamera* ASimHUD_Differential::getSubwindowCamera(int window_index)
{
	return subwindow_cameras_[window_index]; //TODO: index check
}

void ASimHUD_Differential::setSubwindowCamera(int window_index, APIPCamera* camera)
{
	subwindow_cameras_[window_index] = camera; //TODO: index check
	updateWidgetSubwindowVisibility();
}

bool ASimHUD_Differential::getSubwindowVisible(int window_index)
{
	return getSubWindowSettings().at(window_index).visible;
}

void ASimHUD_Differential::setSubwindowVisible(int window_index, bool is_visible)
{
	getSubWindowSettings().at(window_index).visible = is_visible;
	updateWidgetSubwindowVisibility();
}

void ASimHUD_Differential::updateWidgetSubwindowVisibility()
{
...
bool ASimHUD_Differential::isWidgetSubwindowVisible(int window_index)
{
	return widget_->getSubwindowVisibility(window_index) != 0;
}

void ASimHUD_Differential::inputEventToggleSubwindow0()
{
	getSubWindowSettings().at(0).visible = !getSubWindowSettings().at(0).visible;
	updateWidgetSubwindowVisibility();
}

void ASimHUD_Differential::inputEventToggleSubwindow1()
{
	getSubWindowSettings().at(1).visible = !getSubWindowSettings().at(1).visible;
	updateWidgetSubwindowVisibility();
}

void ASimHUD_Differential::inputEventToggleSubwindow2()
{
	getSubWindowSettings().at(2).visible = !getSubWindowSettings().at(2).visible;
	updateWidgetSubwindowVisibility();
}

void ASimHUD_Differential::inputEventToggleAll()
{
	getSubWindowSettings().at(0).visible = !getSubWindowSettings().at(0).visible;
...
void ASimHUD_Differential::createMainWidget()
{
	//create main widget
	...
	if (simmode_)
		widget_->setReportVisible(simmode_->EnableReport);
	widget_->setOnToggleRecordingHandler(std::bind(&ASimHUD_Differential::toggleRecordHandler, this));
	widget_->setRecordButtonVisibility(AirSimSettings::singleton().is_record_ui_visible);
	updateWidgetSubwindowVisibility();
}

void ASimHUD_Differential::setUnrealEngineSettings()
{
...
void ASimHUD_Differential::setupInputBindings()
{
	UAirBlueprintLib::EnableInput(this);

	UAirBlueprintLib::BindActionToKey("inputEventToggleRecording", EKeys::R, this, &ASimHUD_Differential::inputEventToggleRecording);
	UAirBlueprintLib::BindActionToKey("InputEventToggleReport", EKeys::Semicolon, this, &ASimHUD_Differential::inputEventToggleReport);
	UAirBlueprintLib::BindActionToKey("InputEventToggleHelp", EKeys::F1, this, &ASimHUD_Differential::inputEventToggleHelp);
	UAirBlueprintLib::BindActionToKey("InputEventToggleTrace", EKeys::T, this, &ASimHUD_Differential::inputEventToggleTrace);

	UAirBlueprintLib::BindActionToKey("InputEventToggleSubwindow0", EKeys::One, this, &ASimHUD_Differential::inputEventToggleSubwindow0);
	UAirBlueprintLib::BindActionToKey("InputEventToggleSubwindow1", EKeys::Two, this, &ASimHUD_Differential::inputEventToggleSubwindow1);
	UAirBlueprintLib::BindActionToKey("InputEventToggleSubwindow2", EKeys::Three, this, &ASimHUD_Differential::inputEventToggleSubwindow2);
	UAirBlueprintLib::BindActionToKey("InputEventToggleAll", EKeys::Zero, this, &ASimHUD_Differential::inputEventToggleAll);
}

void ASimHUD_Differential::initializeSettings()
{
	...
	AirSimSettings::singleton().load(std::bind(&ASimHUD_Differential::getSimModeFromUser, this));
...
const std::vector<ASimHUD_Differential::AirSimSettings::SubwindowSetting>& ASimHUD_Differential::getSubWindowSettings() const
{
	return AirSimSettings::singleton().subwindow_settings;
}

std::vector<ASimHUD_Differential::AirSimSettings::SubwindowSetting>& ASimHUD_Differential::getSubWindowSettings()
{
	return AirSimSettings::singleton().subwindow_settings;
}

std::string ASimHUD_Differential::getSimModeFromUser()
{
...
void ASimHUD_Differential::loadLevel()
{
...
void ASimHUD_Differential::createSimMode()
{
	...
	if (simmode_name == AirSimSettings::kSimModeTypeMultirotor)
		...
		simmode_ = this->GetWorld()->SpawnActor<ASimModeDifferentialCar>(FVector::ZeroVector,
			FRotator::ZeroRotator,
			simmode_spawn_params);
...
void ASimHUD_Differential::initializeSubWindows()
{
	if (!simmode_)
		return;
...
FString ASimHUD_Differential::getLaunchPath(const std::string& filename)
{
...
bool ASimHUD_Differential::getSettingsText(std::string& settingsText)
{
...
bool ASimHUD_Differential::getSettingsTextFromCommandLine(std::string& settingsText)
{
...
bool ASimHUD_Differential::readSettingsTextFromFile(const FString& settingsFilepath, std::string& settingsText)
{
